...
(function () {
  function initPasswordToggle() {
    ...
    // set initial icon (in case markup changed)
    ...
    toggle.addEventListener('click', function (e) {
      ...
      // keep focus on input after toggle for smooth UX
      input.focus({ preventScroll: true });
    });

    // optional: allow "hold to view" if user presses and holds (uncomment if wanted)
    // toggle.addEventListener('mousedown', () => input.type = 'text');
    // document.addEventListener('mouseup', () => { if (document.activeElement === input) input.type = 'password'; });

    // safety: if something else toggles the input type, keep icon in sync
    const observer = new MutationObserver(() => {
      if (input.type === 'password') toggle.innerHTML = eyeSVG;
